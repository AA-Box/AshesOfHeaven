...
| --- | --- | --- | --- | --- |
| Launch | UNTESTED | PASS (packaged Shipping app booted and ticked with Null RHI; 2026-08-21) | BLOCKED BY TOOLCHAIN | BLOCKED BY TOOLCHAIN |
...
The Mac host currently has the UE 5.8 Apple Silicon toolchain. The source/editor build completed successfully and the packaged app passed `codesign --verify --deep --strict`. Windows, Android, and signed iOS artifacts require their corresponding external build/signing environments. Interactive combat, AI, touch, and full end-to-end checkpoint progression still need a human play session; they remain `UNTESTED` here until that evidence is recorded.
