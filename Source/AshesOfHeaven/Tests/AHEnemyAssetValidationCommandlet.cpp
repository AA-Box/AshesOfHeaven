...
bool UAHEnemyAssetValidationCommandlet::ValidateEnemyAssetManifest(FOutputDevice& Output, int32& OutValidatedAssets)
{
	...
	for (const FName RequiredName : { FName(TEXT("Pilgrim")), FName(TEXT("Warden")), FName(TEXT("Hound")), FName(TEXT("Spider")) })
	{
	...
	for (const FName RequiredName : { FName(TEXT("PilgrimPatrol")), FName(TEXT("PilgrimWarden")) })
	{
