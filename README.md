...
The editor startup map and packaged game default map are both `/Game/ChapterOne/L_ChapterOne_Greybox`. Despite its historical asset name, it streams the authored Erebus, Transit and Cathedral presentation levels and runs **FOR A WHILE**, the first campaign chapter. The front end and pause menu include a mission briefing and squad context. See [the presentation pass](Docs/PROFESSIONAL_PRESENTATION_PASS.md) for changes and remaining production work. Development-only console commands include `AIDebug`, `ObjectiveDebug`, `ReloadCheckpoint`, `RestartEncounter`, `KillAllEnemies`, `SpawnPilgrim`, and `TeleportToEncounter`.
